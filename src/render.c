...
static void render_piece(piece p, int x, int y, float scale)
{
  switch (p)
  {
    case W_KNIGHT:
      DrawTextureEx(assets.w_knight, (Vector2){x, y}, 0, scale, WHITE);
      break;

    case W_PAWN:
      DrawTextureEx(assets.w_pawn, (Vector2){x, y}, 0, scale, WHITE);
      break;

    case W_ROOK:
      DrawTextureEx(assets.w_rook, (Vector2){x, y}, 0, scale, WHITE);
      break;

    case W_BISHOP:
      DrawTextureEx(assets.w_bishop, (Vector2){x, y}, 0, scale, WHITE);
      break;

    case W_QUEEN:
      DrawTextureEx(assets.w_queen, (Vector2){x, y}, 0, scale, WHITE);
      break;

    case W_KING:
      DrawTextureEx(assets.w_king, (Vector2){x, y}, 0, scale, WHITE);
      break;



    case B_KNIGHT:
      DrawTextureEx(assets.b_knight, (Vector2){x, y}, 0, scale, WHITE);
      break;

    case B_PAWN:
      DrawTextureEx(assets.b_pawn, (Vector2){x, y}, 0, scale, WHITE);
      break;

    case B_ROOK:
      DrawTextureEx(assets.b_rook, (Vector2){x, y}, 0, scale, WHITE);
      break;

    case B_BISHOP:
      DrawTextureEx(assets.b_bishop, (Vector2){x, y}, 0, scale, WHITE);
      break;

    case B_QUEEN:
      DrawTextureEx(assets.b_queen, (Vector2){x, y}, 0, scale, WHITE);
      break;

    case B_KING:
      DrawTextureEx(assets.b_king, (Vector2){x, y}, 0, scale, WHITE);
      break;
...
static void render_pieces()
{
  for (int i = 0; i < 8; i++)
  {
    for (int j = 0; j < 8; j++)
    {
      ...
      render_piece(p, j * CELL_SIZE + FRAME_SIZE, i * CELL_SIZE + FRAME_SIZE, 0.5f);
    }
  }
...
static void render_captured_pieces()
{
  for (int i = 0; i < game.w_captured_top; i++)
  {
    ...
    render_piece(p, i * 20 + FRAME_SIZE, CELL_SIZE * 8 + FRAME_SIZE, 0.2f);
  }
  for (int i = 0; i < game.b_captured_top; i++)
  {
    piece p = game.b_captured[i];
    render_piece(p, i * 20 + FRAME_SIZE, 0, 0.2f);
  }
}
